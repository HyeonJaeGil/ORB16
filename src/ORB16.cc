...
static void HarrisResponses(const Mat &img, const std::vector<Rect> &layerinfo,
  ...
  for (ptidx = 0; ptidx < ptsize; ptidx++) {
    ...
    const uchar *ptr0 = ptr00 + (y0 - r + layerinfo[z].y) * size_t_step +
                        (x0 - r + layerinfo[z].x);
    int a = 0, b = 0, c = 0;
...
static void ICAngles(const Mat &img, const std::vector<Rect> &layerinfo,
                     std::vector<KeyPoint> &pts, const std::vector<int> &u_max,
                     int half_k) {
  ...
  for (ptidx = 0; ptidx < ptsize; ptidx++) {
    ...
    const uchar *center = &img.at<uchar>(cvRound(pts[ptidx].pt.y) + layer.y,
...
static void computeOrbDescriptors(const Mat &imagePyramid,
                                  const std::vector<Rect> &layerInfo,
...
#define GET_VALUE(idx)                                                         \
  (x = pattern[idx].x * a - pattern[idx].y * b,                                \
   y = pattern[idx].x * b + pattern[idx].y * a, ix = cvRound(x),               \
   iy = cvRound(y), *(center + iy * step + ix))
#else
#define GET_VALUE(idx)                                                         \
  (x = pattern[idx].x * a - pattern[idx].y * b,                                \
   y = pattern[idx].x * b + pattern[idx].y * a, ix = cvFloor(x),               \
   iy = cvFloor(y), x -= ix, y -= iy,                                          \
   cvRound(center[iy * step + ix] * (1 - x) * (1 - y) +                        \
           center[(iy + 1) * step + ix] * (1 - x) * y +                        \
           center[iy * step + ix + 1] * x * (1 - y) +                          \
           center[(iy + 1) * step + ix + 1] * x * y))
#endif
...
static void initializeOrbPattern(const Point *pattern0,
                                 std::vector<Point> &pattern, int ntuples,
...
/** Compute the ORB16 keypoints on an image
 * @param image_pyramid the image pyramid to compute the features and
 * descriptors on
 * @param mask_pyramid the masks to apply at every level
 * @param keypoints the resulting keypoints, clustered per level
 */
static void
computeKeyPoints(const Mat &imagePyramid, const UMat &uimagePyramid,
                 const Mat &maskPyramid, const std::vector<Rect> &layerInfo,
                 const UMat &ulayerInfo, const std::vector<float> &layerScale,
  ...
  // edge_threshold_ = std::max(edge_threshold_, patch_size_/2 + kKernelWidth /
  // 2 + 2);
  ...
  for (level = 0; level < nlevels; level++) {
    ...
    KeyPointsFilter::retainBest(keypoints, scoreType == ORB16::HARRIS_SCORE
                                               ? 2 * featuresNum
                                               : featuresNum);
    ...
    std::copy(keypoints.begin(), keypoints.end(),
              std::back_inserter(allKeypoints));
  }
  ...
  if (scoreType == ORB16::HARRIS_SCORE) {
    ...
    for (level = 0; level < nlevels; level++) {
      ...
      std::copy(allKeypoints.begin() + offset,
                allKeypoints.begin() + offset + nkeypoints, keypoints.begin());
      offset += nkeypoints;
      ...
      std::copy(keypoints.begin(), keypoints.end(),
                std::back_inserter(newAllKeypoints));
    }
...
void ORB16::detect(InputArray image, std::vector<KeyPoint> &keypoints,
                   InputArray mask) {
  if (image.empty()) {
    keypoints.clear();
    return;
...
void ORB16::detect(InputArrayOfArrays images,
  ...
  if (images.isMatVector()) {
    for (int i = 0; i < nimages; i++) {
      detect(images.getMat(i), keypoints[i],
             masks.empty() ? noArray() : masks.getMat(i));
    }
  } else {
    // assume UMats
    for (int i = 0; i < nimages; i++) {
      detect(images.getUMat(i), keypoints[i],
             masks.empty() ? noArray() : masks.getUMat(i));
    }
  }
}
 ...
 */
void ORB16::compute(InputArray image, std::vector<KeyPoint> &keypoints,
...
void ORB16::compute(InputArrayOfArrays images,
                    std::vector<std::vector<KeyPoint>> &keypoints,
  ...
  if (descriptors.isMatVector()) {
    ...
    CV_Error(Error::StsBadArg,
             "descriptors must be vector<Mat> or vector<UMat>");
  }
}

/** Compute the ORB16 features and descriptors on an image
 * @param img the image to compute the features and descriptors on
 * @param mask the mask to apply
 * @param keypoints the resulting keypoints
 * @param descriptors the resulting descriptors
 * @param do_keypoints if true, the keypoints are computed, otherwise used as an
 * input
 * @param do_descriptors if true, also computes the descriptors
 */
void ORB16::detectAndCompute(InputArray _image, InputArray _mask,
                             std::vector<KeyPoint> &keypoints,
  ...
  if (!do_keypoints) {
    // if we have pre-computed keypoints, they may use more levels than it is
    // set in parameters
    // !!!TODO!!! implement more correct method, independent from the used
    // keypoint detector. Namely, the detector should provide correct size of
    // each keypoint. Based on the keypoint size and the algorithm used (i.e.
    // BRIEF, running on 31x31 patches) we should compute the approximate
    // scale-factor that we need to apply. Then we should cluster all the
    // computed scale-factors and for each cluster compute the corresponding
    // image.
    //
  ...
  Size bufSize((int)alignSize(level0_width + border * 2, 16),
               0); // TODO change alignment to 64
  ...
  for (level = 0; level < nLevels; ++level) {
    ...
    Size wholeSize(sz.width + border * 2, sz.height + border * 2);
    Rect wholeLinfo = Rect(linfo.x - border, linfo.y - border, wholeSize.width,
                           wholeSize.height);
  ...
  if (do_keypoints) {
    ...
    computeKeyPoints(imagePyramid, uimagePyramid, maskPyramid, layerInfo,
    ...
    if (!sortedByLevel) {
      ...
      keypoints.clear();
      for (level = 0; level < nLevels; level++)
        std::copy(allKeypoints[level].begin(), allKeypoints[level].end(),
