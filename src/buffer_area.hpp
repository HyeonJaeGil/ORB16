...
memory sanitizer support or in runtime with the environment variable
...
public:
  ...
  */
  ...
  as the operator `new`, except allocation can be performed later during the BufferArea::commit
  call.
  ...
  */
  template <typename T> void allocate(T *&ptr, size_t count, ushort alignment = sizeof(T)) {
    CV_Assert(ptr == NULL);
    CV_Assert(count > 0);
  ...
  */
  template <typename T> void zeroFill(T *&ptr) {
    CV_Assert(ptr);
    zeroFill_((void **)&ptr);
  }
